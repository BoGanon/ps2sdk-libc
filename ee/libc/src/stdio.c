...
int fclose(FILE *stream)
{
  ...
  switch(stream->type) {
    ...
    case STD_IOBUF_TYPE_STDOUTHOST:
      ...
      break;
    default:
      if ((stream->fd >= 0) && (ret=close(stream->fd) >= 0)) {
  ...
  if ((fname != NULL) && (*fname != '\0')) {
    if ((mode != NULL) && (*mode != '\0')) {
      ...
      if (i < _NFILE) {
        ...
        if ((fd = open((char *)t_fname, iomode, 0666)) >= 0) {
          __iob[i].fd = fd;
          __iob[i].cnt = 0;
          __iob[i].flag = flag;
          ...
          if (!((t_fname[fname_len - 2] == ';') && (t_fname[fname_len - 1] == '1'))) {
            ...
            if ((fd = open((char *)cd_fname, iomode, 0666)) >= 0) {
              __iob[i].fd = fd;
              __iob[i].cnt = 0;
              __iob[i].flag = flag;
              __iob[i].has_putback = 0;
...
size_t fwrite(const void *buf, size_t r, size_t n, FILE *stream)
{
  ...
  switch(stream->type) {
    case STD_IOBUF_TYPE_GS:
      ...
      break;
    case STD_IOBUF_TYPE_STDOUTHOST:
      written = (write(1, (void *) buf, (int)(r * n)) / (int)r);
      break;
    case STD_IOBUF_TYPE_SIO:
    ...
    default:
      ...
      written = (write(stream->fd, (void *)buf, (int)(r * n)) / (int)r);
  }
  if (written < 0) {
